...
model_chose = [0;0;0;1;1;1];
num = ['0.0';'0.3';'0.6';'0.9';'1.2';'1.5'];
for n = 1:6
    subplot(2,3,n);
    str=strcat('disturbance=',num(n,:));
    gyq_str=strcat('gyq_',num(n,:),'.csv');
    wlh_str=strcat('wlh_',num(n,:),'.csv');
    yyp_str=strcat('yyp_',num(n,:),'.csv');
    zrj_str=strcat('zrj_',num(n,:),'.csv');
    chj_str=strcat('chj_',num(n,:),'.csv');
